...
// ReAssignCob resets the role's COB assignment and assigns new COBs to the role.
//
// Parameters:
// - id: the UUID of the role.
// - cobReq: a slice of UUIDs representing the COBs to be assigned to the role.
//
// Returns:
// - error: an error if the deletion or insertion operation fails.
func (repo *roleRepository) ReAssignCob(id uuid.UUID, cobReq []uuid.UUID) error {
	// reset role cobs assignment
	query := `
		DELETE FROM roles_cobs
		WHERE role_id = $1
	`
	// Execute the query and delete requested row.
	_, err := repo.Conn.Exec(query, id)
	...
	// assign cob to role, by create new pivot entry that on roles_cobs
	for _, cobId := range cobReq {
		// assign cob group to role
		_, err := repo.Conn.Exec(`INSERT INTO roles_cobs
				(cob_id, role_id)
			VALUES
				($1, $2)`,
			cobId,
		...
		if err != nil {
			fmt.Println("Error scanning row Role:", err)
			return err
		}
	}
...
// ReAssignCategory resets the role categories assignment and assigns new categories to the role.
//
// Parameters:
// - id: the UUID of the role.
// - categoryReq: a slice of UUIDs representing the categories to be assigned to the role.
//
// Returns:
// - error: an error if there was a problem executing the database queries.
func (repo *roleRepository) ReAssignCategory(id uuid.UUID, categoryReq []uuid.UUID) error {
	// reset role categories assignment
	query := `
	...
	// assign category to role, by create new pivot entry that on roles_categories
	for _, categoryId := range categoryReq {
		// assign category group to role
		_, err := repo.Conn.Exec(`INSERT INTO roles_categories
				(category_id, role_id)
			VALUES
				($1, $2)`,
			categoryId,
