...
func (repo *authRepository) RequestResetPassword(email string) error {
	// get user by email
...
func (repo *authRepository) GetUserByResetPasswordToken(token string) (user models.User, errorMain error) {
	...
	err := repo.Conn.QueryRow(query, token).Scan(&user.ID, &user.Email)
...
func (repo *authRepository) DestroyResetPasswordToken(token string) error {
...
// Returns:
// - error: An error if the deletion fails, nil otherwise.
func (repo *authRepository) DestroyAllResetPasswordToken(userId uuid.UUID) error {
	// SQL query to delete the user with the given access token.
	query := `
		DELETE FROM reset_password_tokens
		WHERE user_id = $1
...
func (repo *authRepository) IncreasePasswordExpiredAt(userId uuid.UUID) error {
	...
	`
	// Execute the query and delete requested row.
	_, err := repo.Conn.Exec(query, userId, expiredAt)
