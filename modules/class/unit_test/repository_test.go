...
func TestCreateRepository(t *testing.T) {
	...
	// create table
	err = classRepo.CreateTable("D:/ngoding/roketin/tugure-roketin/v2/blips-v2-backend/database/migrations/create_class_table.up.sql")
	t.Log(err)

	res, err := classRepo.CreateClass(class1)
...
func TestGetRepository(t *testing.T) {
	...
	resAll, total, err := classRepo.GetIndexClass(request.PageRequest{
		Page:    1,
		PerPage: 2,
	})

	// resAll, err := classRepo.GetAllClass()

	t.Log(err)
	t.Log(total)
	t.Log(resAll)

	// resOne, err := classRepo.GetClassByID(resAll[0].ID)
