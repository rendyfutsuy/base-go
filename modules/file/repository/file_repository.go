...
func (r *fileRepository) AssignFilesToModule(ctx context.Context, moduleType string, moduleID uuid.UUID, fileIDs []uuid.UUID) error {
	if len(fileIDs) == 0 {
		return nil
	}
	now := time.Now().UTC()
	items := make([]models.FilesToModule, 0, len(fileIDs))
