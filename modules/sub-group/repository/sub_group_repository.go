...
func (r *subGroupRepository) Create(ctx context.Context, goodsGroupID uuid.UUID, name string, createdBy string) (*models.SubGroup, error) {
	...
	if err := r.DB.WithContext(ctx).Select("id", "goods_group_id", "subgroup_code", "name", "created_at", "created_by", "updated_at", "updated_by").Where("id = ?", sg.ID).First(sg).Error; err != nil {
		return nil, err
	}
	return sg, nil
}
...
func (r *subGroupRepository) ExistsByName(ctx context.Context, goodsGroupID uuid.UUID, name string, excludeID uuid.UUID) (bool, error) {
	var count int64
	q := r.DB.WithContext(ctx).Unscoped().Model(&models.SubGroup{}).Where("goods_group_id = ? AND name = ?", goodsGroupID, name)
...
func (r *subGroupRepository) GetAll(ctx context.Context, filter dto.ReqSubGroupIndexFilter) ([]models.SubGroup, error) {
	...
	query = request.ApplySearchCondition(query, filter.Search, []string{"sg.name", "sg.subgroup_code"})
